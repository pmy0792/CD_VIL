...
class MNIST_RGB(datasets.MNIST):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        domain_idx = 0
        class_group = int(target/2)
        

        return img, target, domain_idx, class_group
...
class CORe50(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,args=None):        
        ...
        if self.train:
            fpath = self.fpath + '/train'
            self.data = [CustomImageFolder(f'{fpath}/{s}', transform=transform, args=args) for s in self.train_session_list]
        else:
            fpath = self.fpath + '/test'
            self.data = CustomImageFolder(fpath, transform=transform, args=args)
...
class DomainNet(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, args=None):
        ...
        if not os.path.exists(self.root + '/train') and not os.path.exists(self.root + '/test'):
            for i in range(len(self.fpath)):
                if not os.path.exists(os.path.join(self.root, self.filename[i][:-4])):
                    import zipfile
                    print(os.path.join(self.root, self.filename[i]))
        ...
        domain_order = ["real","sketch","infograph","painting","quickdraw","clipart"]
        if self.train:
            fpath = self.root + '/train'
            self.data = [CustomImageFolder(f'{fpath}/{d}', transform=transform,args=args) for d in domain_order]
        else:
...
class CustomImageFolder(datasets.ImageFolder):
    def init_class_group(self,args):
        self.class_group_dict=dict()
        
        class_group_size = args.class_group_size
        num_class_group = int(args.num_classes / args.class_group_size)
        
        for i in range(num_class_group):
            self.class_group_dict[i] = [k for k in range(class_group_size*i, class_group_size*(i+1))]
    
    def __init__(self, root, transform=None, target_transform=None,args=None):
        
        super().__init__(root, transform, target_transform) # root: f"{fpath}/{d}"
        self.domain_indices={}
        if args.dataset=="DomainNet":
            self.domain_indices = {
                "real":0, "sketch":1, "infograph":2, "painting":3, "quickdraw":4, "clipart":5,"test":6
            }
            if root.split('/')[-1]=="test":
                self.domain_idx=-1
            self.domain_idx = self.domain_indices[root.split('/')[-1]]
        elif args.dataset=="CORe50":
            for i in range(11):
                self.domain_indices[f"s{i+1}"] = i
            if root.split('/')[-1]=="test":
                self.domain_idx=-1
            else:
                self.domain_idx = self.domain_indices[root.split('/')[-1]]
        self.init_class_group(args)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        for key in self.class_group_dict.keys():
            if target in self.class_group_dict[key]:
                class_group = key
        
        return sample, target, self.domain_idx, class_group