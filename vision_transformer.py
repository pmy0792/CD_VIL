...
from prompt import EPrompt, SPrompt, DPrompt
from attention import PreT_Attention

from vit_module.PatchEmbed import PatchEmbed
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            use_d_prompt=False, d_prompt_top_k=1, d_prompt_length=10, d_prompt_as_dualprompt=False,
            image_encoder=None,num_tasks=-1,
            args=None,
            ):
        ...
        self.d_prompt_as_dualprompt = d_prompt_as_dualprompt

        
        #! D-prompt
        if self.use_d_prompt and not self.d_prompt_as_dualprompt:
            self.d_prompt_top_k = d_prompt_top_k  
            self.d_prompt=DPrompt(length=d_prompt_length, top_k=self.d_prompt_top_k,embed_dim=embed_dim,pool_size=args.num_tasks,epochs=self.epochs)    
            self.image_encoder = image_encoder
            
        elif self.use_d_prompt and self.d_prompt_as_dualprompt:
            self.image_encoder = image_encoder
            dg_prompt_shape=(num_g_prompt, 2, g_prompt_length, num_heads, embed_dim // num_heads)
            self.dg_prompt = nn.Parameter(torch.randn(dg_prompt_shape))
            print("Created D-G Prompt")
            nn.init.uniform_(self.dg_prompt, -1, 1)
            
            self.de_prompt = EPrompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
        ...
        if self.use_g_prompt:
            g_prompt_shape=(num_g_prompt, 2, g_prompt_length, num_heads, int(embed_dim // num_heads))
        ...
        self.total_prompt_len = 0 # if prefix-> use only class token in classification head
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False, upper=False, correct_d=None, epoch=-1, embed_patch=True, dg_prompt=None, de_prompt=None):
        image_tokens=x
        if embed_patch:
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            original_x=x
            if self.use_d_prompt and not self.d_prompt_as_dualprompt:
                if self.args.de_loss and train: #! extract de_cls_feature for every e-prompt
                            _x=original_x
                            GE_cls_features_list=[]
                           
                            for task in range(task_id+1):
                                g_prompt_counter = -1
                                e_prompt_counter = -1
                                task = torch.tensor([task]).expand(_x.shape[0], -1).contiguous() # B, top_k 
                                batched_prompt_raw = self.e_prompt.prompt[:,:,task]
                                ...
                                for i, block in enumerate(self.blocks):
                                    if i in self.g_prompt_layer_idx:
                                        if self.use_prefix_tune_for_g_prompt:
                                            ...
                                            idx = torch.tensor([g_prompt_counter] * _x.shape[0]).to(_x.device)
                                            g_prompt = self.g_prompt[idx]
                                        else:
                                            g_prompt=None
                                        _x = block(_x, prompt=g_prompt)
                                
                                    elif i in self.e_prompt_layer_idx:
                                        e_prompt_counter += 1
                                        if self.use_prefix_tune_for_e_prompt:
                                            # Prefix tunning, [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
                                            _x = block(_x, prompt=e_prompt_res[e_prompt_counter])
                                        else:
                                            # Prompt tunning, [B, top_k * e_prompt_length, embed_dim]
                                            prompt = e_prompt_res[e_prompt_counter]
                                            _x = torch.cat([prompt, _x], dim=1)
                                            _x = block(_x)
                                    else:
                                        _x = block(_x)
                                GE_cls_features = _x[:, 0] # b, embed_dim
                                GE_cls_features_list.append(GE_cls_features)
                            res['GE_cls_features_list']=GE_cls_features_list
                            
                if train or correct_d:#train or upper:
                    start = task_id * self.d_prompt.top_k
                    end = (task_id + 1) * self.d_prompt.top_k
                    single_prompt_mask = torch.arange(start, end).to(x.device)
                    prompt_mask = single_prompt_mask.unsqueeze(0).expand(x.shape[0], -1)
                    if end > self.d_prompt.pool_size:
                        prompt_mask = None
                else: #test
                    prompt_mask = None
                    
                res.update(self.d_prompt(original_x, prompt_mask=prompt_mask, cls_features=cls_features))
                
                x_with_d_prompt = res['prompted_embedding']
                encoder_res= self.image_encoder(x_with_d_prompt,embed_patch=False)
                d_cls_features = encoder_res['pre_logits'] # b, embed_dim
                res['d_cls_features'] = d_cls_features
                ...
                if self.use_g_prompt and self.use_e_prompt:
                    g_prompt_counter = -1
                    e_prompt_counter = -1
                    if not train: # test time
                        prompt_mask=None
                    res_ = self.e_prompt(original_x, prompt_mask=prompt_mask, cls_features=cls_features, e_upper=upper)
                    e_prompt_res = res_['batched_prompt']
                    res['e_reduce_sim']=res_['e_reduce_sim']
                    ...
                    for i, block in enumerate(self.blocks):
                        if i in self.g_prompt_layer_idx:
                            ...
                            if self.use_prefix_tune_for_e_prompt:
                                # Prefix tunning, [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
                                x = block(x, prompt=e_prompt_res[e_prompt_counter])
                            else:
                                # Prompt tunning, [B, top_k * e_prompt_length, embed_dim]
                                prompt = e_prompt_res[e_prompt_counter]
                ...
                x=self.norm(x)
                GE_cls_features = x[:, 0] # b, embed_dim
                res['GE_cls_features']=GE_cls_features   
                res['x']=x
                return res
                 
            elif de_prompt != None and dg_prompt != None: #! for extracting d_cls_features
                    ...
                    for i, block in enumerate(self.blocks):
                        if i in [0,1]:
                                ...
                                # Prefix tunning, [B, 2, g_prompt_length, num_heads, embed_dim // num_heads]
                                idx = torch.tensor([g_prompt_counter] * x.shape[0]).to(x.device)
                                g_prompt = dg_prompt[idx]
                                x = block(x, prompt=g_prompt)
                    
                        elif i in [2,3,4]:
                                e_prompt_counter += 1
                                # Prefix tunning, [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
                                x = block(x, prompt=de_prompt)
                        else:
                            x = block(x)
                    x=self.norm(x)
                    res['x']=x
                    return res
            #!!   
            elif self.use_d_prompt and self.d_prompt_as_dualprompt and self.use_g_prompt and self.use_e_prompt: 
                if (self.use_prompt_mask and train) or upper:
                    start = task_id * self.e_prompt.top_k # top_k=1
                ...
                _res = self.e_prompt(x, prompt_mask=prompt_mask, cls_features=cls_features, e_upper=upper)
                e_prompt_res = _res['batched_prompt']
                res['e_reduce_sim'] = _res['e_reduce_sim']
                _res = self.de_prompt(x, prompt_mask=prompt_mask, cls_features=cls_features, e_upper=upper)
                de_prompt_res = _res['batched_prompt']
                res['de_reduce_sim'] = _res['e_reduce_sim']

                encoder_res= self.image_encoder(image_tokens,de_prompt=de_prompt_res,dg_prompt=self.dg_prompt)
                
                res['d_cls_features']=encoder_res['pre_logits'] # b, embed_dim
                for i, block in enumerate(self.blocks):
                    if i in self.g_prompt_layer_idx:
                        if self.use_prefix_tune_for_g_prompt:
                            g_prompt_counter += 1
                            ...
                            idx = torch.tensor([g_prompt_counter] * x.shape[0]).to(x.device)
                            g_prompt = self.g_prompt[idx]
                            dg_prompt = self.dg_prompt[idx]
                        else:
                            g_prompt=None
                        g_prompt_total = torch.cat([g_prompt,dg_prompt],dim=2)
                        x = block(x, prompt=g_prompt_total)
                    
                    elif i in self.e_prompt_layer_idx:
                        e_prompt_counter += 1
                        if self.use_prefix_tune_for_e_prompt:
                            # Prefix tunning, [B, 2, top_k * e_prompt_length, num_heads, embed_dim // num_heads]
                            e_prompt_total = torch.cat([e_prompt_res[e_prompt_counter],de_prompt_res[e_prompt_counter]],dim=2)
                            x = block(x, prompt=e_prompt_total)
                    else:
                        x = block(x)
            else:
                x = self.blocks(x)
    ...
    def forward_head(self, res, pre_logits: bool = False, train=False, task_id=-1,args=None):
            x = res['x'] # (bs, token_len, embed_dim)
            ...
            x = self.fc_norm(x)
            res['logits'] = self.head(x) # x.shape: (bs, embed_dim)
        
            return res

    def forward(self, x, task_id=-1, cls_features=None, train=False, upper=False,correct_d=None,args=None,embed_patch=True, de_prompt=None,dg_prompt=None):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train, upper=upper, correct_d=correct_d,embed_patch=embed_patch)
        res = self.forward_head(res, train=train,task_id=task_id, args=args)
        return res
